...
const links = [
  { label: 'My progress', href: '/history', external: false },
  ...
  return (
    <footer className="creator-signature">
      <style>{`
        ...
        .creator-signature .made{color:#6E6A5C}.creator-signature .sep{color:#C9C3B2;user-select:none}.creator-signature a{color:#6E6A5C;text-decoration:none;text-underline-offset:3px;transition:color .15s ease}.creator-signature a:hover,.creator-signature a:focus-visible{color:#1A56A8;text-decoration:underline;outline:none}
      ...
      `}</style>
      <span className="made">Made with ❤️ by Satnam</span><span className="sep">·</span>
      {links.map((link,index)=><React.Fragment key={link.href}><a href={link.href} {...(link.external?{target:'_blank',rel:'noopener noreferrer'}:{})} aria-label={`${link.label}${link.external?' — Satnam Singh':''}`}>{link.label}</a>{index<links.length-1&&<span className="sep">·</span>}</React.Fragment>)}
    </footer>
  )
}
