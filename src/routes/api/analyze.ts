...
function normalizedAudioMime(fileName: string, reportedType: string) {
  ...
  const byExtension: Record<string, string> = {
    m4a: 'audio/m4a', mp3: 'audio/mp3', mpeg: 'audio/mpeg', wav: 'audio/wav', webm: 'audio/webm',
    ogg: 'audio/ogg', opus: 'audio/opus', aac: 'audio/aac', flac: 'audio/flac', aiff: 'audio/aiff', aif: 'audio/aiff',
  }
...
async function analyzeWithGeminiRetry(input: Parameters<typeof analyzeWithGemini>[0]) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try { return await analyzeWithGemini(input) }
    catch (error) {
      lastError = error
      ...
      POST: async ({ request }) => {
        try {
          ...
          for (let index = 1; index < wordTimestamps.length; index += 1) {
            largestRawWordGapSec = Math.max(largestRawWordGapSec, Math.max(0, wordTimestamps[index].start - wordTimestamps[index - 1].end))
          }

          return Response.json({
            transcript, metrics, analysis, scores,
