...
function readContext(request: Request) {
  try {
    const raw = cookieValue(request, 'peitho_live_ctx')
    if (!raw) return ''
    const parsed = JSON.parse(decodeURIComponent(raw))
    if (!parsed?.text || !parsed?.ts || Date.now() - Number(parsed.ts) > 12_000) return ''
    return String(parsed.text).slice(-900)
  } catch { return '' }
}

function contextCookie(text: string) {
  const value = encodeURIComponent(JSON.stringify({ ts: Date.now(), text: text.slice(-900) }))
  return `peitho_live_ctx=${value}; Path=/; Max-Age=900; SameSite=Lax; Secure`
}

function wavRms(buffer: ArrayBuffer) {
  if (buffer.byteLength < 48) return 1
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const mime = liveMime(fileName, entry.type)
          if (mime === 'audio/wav' && wavRms(bytes) < 0.0045) {
            return Response.json({ transcript: '' }, { headers: { 'Cache-Control': 'no-store' } })
          }

          const previous = readContext(request)
          const audio = new Blob([bytes], { type: mime })
          const groq = new FormData()
          groq.append('file', audio, fileName)
          groq.append('model', process.env.GROQ_LIVE_STT_MODEL?.trim() || 'whisper-large-v3')
          groq.append('language', 'en')
          groq.append('temperature', '0')
          groq.append('response_format', 'json')
          if (previous) groq.append('prompt', previous)

          const response = await fetch('https://api.groq.com/openai/v1/audio/transcriptions', {
            method: 'POST', headers: { Authorization: `Bearer ${key}` }, body: groq,
          })
          if (!response.ok) {
            const detail = await response.text()
            return Response.json({ error: `Groq live transcription failed (${response.status}): ${detail.slice(0, 350)}` }, { status: response.status, headers: { 'Cache-Control': 'no-store' } })
          }
          const payload: any = await response.json()
          const transcript = String(payload?.text || '').trim()
          const nextContext = `${previous} ${transcript}`.trim().slice(-900)
